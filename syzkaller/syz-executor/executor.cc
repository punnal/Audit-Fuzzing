...
void execute_call(thread_t* th)
{
    ...
    number = number + 1;
    /* debug("#zzzzz|%d|%s(0x%llx, 0x%llx, 0x%llx, 0x%llx)\n", number, call->name, (uint64)th->args[0], (uint64)th->args[1], (uint64)th->args[2], (uint64)th->args[3]); */
	/* debug("#%d [%llums] -> %s(", */
